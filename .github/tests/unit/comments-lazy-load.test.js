...
describe('Comments Lazy Loading', () => {
  ...
  describe('Initialization and Container Detection', () => {
    test.each([
      ...
      ['data-lazy-load is not "true"', '<div id="cusdis_thread" data-lazy-load="false"></div>'],
      ['data-lazy-load attribute is missing', '<div id="cusdis_thread"></div>']
    ...
    test('should initialize when container exists with data-lazy-load="true"', () => {
      document.body.innerHTML = '<div id="cusdis_thread" data-lazy-load="true"></div>';
      loadCommentsScript();
      expect(mockIntersectionObserver).toHaveBeenCalled();
    });

    test('should only initialize the comment system that exists on the page', () => {
      document.body.innerHTML = '<div id="disqus_thread" data-lazy-load="true" data-shortname="test"></div>';
      loadCommentsScript();
    ...
    test('should handle multiple comment systems but only initialize those present', () => {
      document.body.innerHTML = `
        <div id="cusdis_thread" data-lazy-load="true"></div>
        <div id="giscus_thread" data-lazy-load="true" data-repo="test/repo"></div>
      `;
      loadCommentsScript();
  ...
  describe('Cusdis Loader', () => {
    ...
    test.each([
      ...
      document.body.innerHTML = `<div id="cusdis_thread" data-lazy-load="true"${langAttr}></div>`;
      loadCommentsScript();
      observerCallback([{ isIntersecting: true }]);
  ...
  describe('Disqus Loader', () => {
    test('should load Disqus script with correct shortname', () => {
      document.body.innerHTML = '<div id="disqus_thread" data-lazy-load="true" data-shortname="mysite"></div>';
      loadCommentsScript();
      observerCallback([{ isIntersecting: true }]);
    ...
    test('should log error when shortname is missing', () => {
      ...
      document.body.innerHTML = '<div id="disqus_thread" data-lazy-load="true"></div>';
      loadCommentsScript();
      observerCallback([{ isIntersecting: true }]);
  ...
  describe('Single Comment System Guarantee', () => {
    ...
    const commentSystems = [
      {
        name: 'Cusdis',
        html: '<div id="cusdis_thread" data-lazy-load="true"></div>',
      ...
      {
        name: 'Disqus',
        html: '<div id="disqus_thread" data-lazy-load="true" data-shortname="test"></div>',
        scriptUrl: null,
        selector: null,
        scriptMatch: 'disqus',
      ...
      {
        name: 'Giscus',
        html: '<div id="giscus_thread" data-lazy-load="true" data-repo="test/repo"></div>',
      ...
      {
        name: 'Utterances',
        html: '<div id="utterances_thread" data-lazy-load="true" data-repo="test/repo" data-issue-term="pathname" data-theme="github-light"></div>',
