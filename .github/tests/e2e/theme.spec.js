...
/**
 * Theme and dark mode tests for Type-on-Strap theme
 */
test.describe('Theme and Dark Mode', () => {
  ...
  test('should have correct theme styles applied', async ({ page }) => {
    ...
    // Check if theme attribute exists
    const theme = await page.getAttribute('html', 'data-theme');
...
/**
 * Dark mode initialization and preference tests
 */
test.describe('Dark Mode Initialization', () => {
  ...
  test('should respect browser preference for dark mode', async ({ browser }) => {
    ...
    // Clear localStorage to test browser preference
    await page.goto('/');
    await page.evaluate(() => localStorage.clear());
    await page.reload();
    await page.waitForTimeout(500);
    ...
    // Should detect dark preference and set dark mode
  ...
  test('should prioritize localStorage over browser preference', async ({ browser }) => {
    ...
    const theme = await page.getAttribute('html', 'data-theme');
    // localStorage should override browser preference
    expect(theme).toBe('light');
  ...
  test('should have correct button text based on theme', async ({ page }) => {
    ...
    if (await themeToggle.isVisible()) {
      // Check light mode shows "Dark" button
      await page.evaluate(() => {
      ...
      // Check dark mode shows "Light" button
      await themeToggle.click();
      await page.waitForTimeout(300);
