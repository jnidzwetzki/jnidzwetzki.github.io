...
Using Container images, the reverse proxy also fulfills a further task. Wenn multiple Containers provide a webinterface, only one container could use the port 80/tcp or 443/tcp and receive the incoming HTTP and HTTPs connections. Another container has to use non-default ports, which might be inconvenient for users (e.g., https://example.com:100000). Using the reverse proxy, the proxy could listen on the default ports and forward the connections based on the URLs to the actual container ports. This is illustrated in the following image:
